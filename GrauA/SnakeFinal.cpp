...
void aplicaTransformacoes(	GLuint shaderID, Geometry elemento, vec3 eixoDeRotacao = vec3(0.0, 0.0, 1.0));
														...
														// 7º parâmetro: eixo no qual se processará a rotação do elemento. Defauld -> eixo "z"
...
float smoothFactor = 1.2f;	// "velocidade" de incremento na posição dos elementos
...
vec3 dir2Mouse = vec3(0.0, -1.0, 0.0); // Vetor direção da cabeça (snake[0]) para o mouse
...
int main () {
	...
	Geometry seed;
	seed.VAO = createSeed ();
	seed.posicao = vec3((int)(WIDTH*rand())/RAND_MAX, (int)(HEIGHT*rand())/RAND_MAX, 0);
	...
	while (!glfwWindowShouldClose(window))	{
        ...
        vec3 mousePos = vec3(xPos, height - yPos, 0.0);  // Inverte o eixo Y para se alinhar à tela
    
	    vec3 dir2Mouse = normalize(mousePos - snake[0].posicao);	// Calcula o vetor direção normalizado
        
		if (addNew) {
			cout << distance(snake[0].posicao, mousePos) << " " << length(mousePos - snake[0].posicao) << endl;
			addNew = false;
		}
*/
    	// Move o elemento suavemente na direção do mouse ou na direção dada pelas teclas
        //if (distance(snake[0].posicao, mousePos)) > 0.1f) { snake[0].posicao += 0.5f * dir2Mouse; } // Aumentar ou diminuir "smoothFactor" para controlar a velocidade
		...
		for (int i = 0; i < snake.size(); i++) {
			...
			if (i==0) { 
				...
				dir2Target = normalize(mousePos - snake[0].posicao);	// Calcula o vetor direção normalizado
				dis2Target = length(mousePos - snake[0].posicao);
			}
			else {
				dir2Target = normalize(snake[i-1].posicao - snake[i].posicao);	// vetor direção normalizado
				dis2Target = length(snake[i-1].posicao - snake[i].posicao);		// distância entre os segmentos
			}

			// Calcula a nova posição do segmento com suavidade, respeitando as distâncias mínima e máxima
			vec3 targetPosition = snake[i].posicao;
			float dynamicSmoothFactor = smoothFactor * (dis2Target / maxSegDistance);

			if      (dis2Target < minSegDistance) { targetPosition = snake[i].posicao + (dis2Target - minSegDistance) * dir2Target; }
			else if (dis2Target > maxSegDistance) { targetPosition = snake[i].posicao + (dis2Target - maxSegDistance) * dir2Target; }
			...
			snake[i].angulo = atan2(dir2Target.y, dir2Target.x) + radians(-90.0f); // Rotaciona para que a ponta aponte para o mouse
		...
		for (int i = snake.size() - 1; i >= 0; i--) {
			aplicaTransformacoes(shaderID, snake[i].VAO, snake[i].posicao, snake[i].angulo, snake[i].dimensao, snake[i].cor);
			glDrawArrays(GL_TRIANGLE_FAN, 0, 34);
		}
	
		// Desenha as escleras dos olhos
		aplicaTransformacoes(shaderID, eyes.VAO, eyes.posicao, eyes.angulo, eyes.dimensao, eyes.cor);
		glDrawArrays(GL_TRIANGLE_FAN,   0, 34);
