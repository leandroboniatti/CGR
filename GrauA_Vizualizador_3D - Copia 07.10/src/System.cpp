...
bool System::loadSceneObjects() {
    ...
    */
    for (auto& config : configs) {
        auto obj = make_unique<OBJ3D>(config.name);
        
        // Try to load the model, if it fails, continue without it
        if (obj->readFile(config.modelPath)) {
            ...
            sceneObjects.push_back(move(obj));
            cout << "Objeto carregado: " << config.name << endl;
        } else {
            cout << "Falha ao carregar objeto: " << config.name 
                 << " de " << config.modelPath << endl;
        }
    }
...
vector<ObjectConfig> System::loadConfiguration() {
    vector<ObjectConfig> configs;

    ifstream configFile("Configurador_Cena.txt");
    if (!configFile.is_open()) {
        std::cout << "Arquivo de configuração da cena não encontrado (Configurador_Cena.txt)" << std::endl;
        return configs;
    }
